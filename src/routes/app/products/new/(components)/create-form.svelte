<script lang="ts">
	...
	import { productCreateSchema, type ProductCreateSchema } from '$lib/types/form.types';
	...
	let {
		...
		validatedForm: SuperValidated<Infer<ProductCreateSchema>>;
		onSuccess: (product_id: string) => void;
		isOpen: boolean;
	} = $props();

	async function onFormUpdated({ form }: { form: SuperValidated<Infer<ProductCreateSchema>> }) {
		const message = form.message as FormMessage<{ product_id: string }> | undefined;
		...
		if (!message) {
			...
			error = undefined;
			onSuccess(message.data.product_id);
		}
	}

	const superForm = appSuperForm(validatedForm, {
		validators: zodClient(productCreateSchema),
...
<Panel>
	{#snippet header()}
		<Header>
			{#snippet heading()}
				<h2 class="font-semibold">Create Product</h2>
			{/snippet}
