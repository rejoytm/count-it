<script lang="ts">
	...
	let {
		...
		validatedForm: SuperValidated<Infer<InvoiceUpdateSchema>>;
		onSuccess: () => void;
		products: Product[];
		salesTaxes: SalesTax[];
		customers: Customer[];
	...
	async function onFormUpdated({ form }: { form: SuperValidated<Infer<InvoiceUpdateSchema>> }) {
	...
	const superForm = appSuperForm(validatedForm, {
		validators: zodClient(invoiceUpdateSchema),
...
<Panel maxWidthClass="max-w-4xl">
	{#snippet header()}
		<Header>
			{#snippet heading()}
				<h2 class="font-semibold">Edit Invoice</h2>
			{/snippet}
				...
				<Form.Control>
					{#snippet children({ props })}
						...
						<div class="space-y-1 leading-none">
							<Form.Label class="text-sm text-foreground">Mark as void invoice</Form.Label>
							<Form.Description>
